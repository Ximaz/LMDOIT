...
class TestCookieJar(unittest.TestCase):
    def test_requests_cookie_jar(self):
        ...
        first_cookie.set_cookie(requests.cookies.create_cookie("login_date", "20230810_135603"))
        auth.cookie(cookie=first_cookie)

        second_cookie = requests.cookies.RequestsCookieJar()
        second_cookie.set_cookie(requests.cookies.create_cookie("last_post_id", "918829937"))
        second_cookie.set_cookie(requests.cookies.create_cookie("last_message_id", "99288839"))
        auth.cookie(cookie=second_cookie)
